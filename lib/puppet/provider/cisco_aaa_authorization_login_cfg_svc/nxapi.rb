#
# The NXAPI provider for cisco_aaa_authorization_login_cfg_svc.
#
# December 2015
#
...
Puppet::Type.type(:cisco_aaa_authorization_login_cfg_svc).provide(:nxapi) do
  desc 'The NXAPI provider for cisco_aaa_authorization_login_cfg_svc.'
  ...
  def flush
    if @property_flush[:ensure] == :absent
      ...
      if @aaa_login_svc.nil?
        @aaa_login_svc = Cisco::AaaAuthorizationService.new(:config_commands,
                                                            @resource[:name])
      end
      properties_set
    end
  ...
  def puts_config
    debug 'Current state:'
    if @aaa_login_svc.nil?
      debug 'No aaa login config service'
      return
    end
    debug "
        name: #{@resource[:name]}
