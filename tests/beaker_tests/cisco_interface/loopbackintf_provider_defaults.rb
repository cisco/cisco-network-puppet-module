...
# TestCase Name:
# -------------
# LoopbackIntf-Provider-Defaults.rb
#
# TestCase Prerequisites:
# -----------------------
# This is a Puppet LOOPBACKINTF resource testcase for Puppet Agent on Nexus devices.
...
# TestCase:
# ---------
# This is a LOOPBACKINTF resource test that tests for default values for
...
# Require UtilityLib.rb and LoopbackIntfLib.rb paths.
require File.expand_path('../../lib/utilitylib.rb', __FILE__)
require File.expand_path('../loopbackintflib.rb', __FILE__)

result = 'PASS'
testheader = 'LOOPBACKINTF Resource :: All Attributes Defaults'

# @test_name [TestCase] Executes defaults testcase for LOOPBACKINTF Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Get resource present manifest from master' do
    # Expected exit_code is 0 since this is a bash shell cmd.
    on(master, LoopbackIntfLib.create_loopbackintf_manifest_present)
  ...
  step 'TestStep :: Check interface instance presence on agent' do
    ...
    cmd_str = get_vshell_cmd('show running-config interface loopback1')
    on(agent, cmd_str) do
      search_pattern_in_output(stdout, [%r{ip address 192.168.1.1/16}],
                               false, self, logger)
    end
