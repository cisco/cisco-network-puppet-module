...
# TestCase Name:
# -------------
# SviIntf-Provider-NonDefaults.rb
#
...
# TestCase:
# ---------
# This is a SVIINTF resource test that tests for nondefault values for
...
result = 'PASS'
testheader = 'SVIINTF Resource :: All Attributes NonDefaults'

# @test_name [TestCase] Executes nondefaults testcase for SVIINTF Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Setup switch for provider test' do
    ...
    cmd_str = get_vshell_cmd('show running-config interface vlan 80')
    on(agent, cmd_str, acceptable_exit_codes: [16]) do
      search_pattern_in_output(stdout, [/interface Vlan80/],
                               true, self, logger)
    end
  ...
  step 'TestStep :: Get resource nondefaults manifest from master' do
    # Expected exit_code is 0 since this is a bash shell cmd.
    on(master, SviIntfLib.create_sviintf_manifest_nondefaults)
  ...
  step 'TestStep :: Check interface instance presence on agent' do
    ...
    on(agent, cmd_str) do
      search_pattern_in_output(stdout,
                               [%r{ip address 192.168.1.1/16}, /management/, /no autostate/],
                               false, self, logger)
    end
