...
tests = {
  agent:         agent,
  master:        master,
  resource_name: 'cisco_aaa_authorization_login_cfg_svc',
  ensurable:     false,
}
...
class TestCiscoAaaLoginCfgSvc < BaseHarness
...
test_name "TestCase :: #{tests[:resource_name]}" do
  ...
  %w(default console).each do |title|
    ...
    test_harness_run(tests, :default_default, harness_class: TestCiscoAaaLoginCfgSvc)
    if tests[:agent]
      generate_default_symbols_test(tests, title)
      test_harness_run(tests, :default_symbols, harness_class: TestCiscoAaaLoginCfgSvc)
    end
    # Absent tests do not set to absent, cannot test idempotency
    create_manifest_and_resource(tests, :default_absent, harness_class: TestCiscoAaaLoginCfgSvc)
  ...
  create_manifest_and_resource(tests, :invalid_name, harness_class: TestCiscoAaaLoginCfgSvc)
  ...
  %w(default console).each do |title|
    ...
    create_manifest_and_resource(tests, :nondefault, harness_class: TestCiscoAaaLoginCfgSvc)
    ...
    create_manifest_and_resource(tests, :nondefault_symbols, harness_class: TestCiscoAaaLoginCfgSvc)
