...
# TestCase Name:
# -------------
# AaaLoginCfgSvc-Provider-Defaults.rb
#
# TestCase Prerequisites:
# -----------------------
# This is a Puppet AaaLoginCfgSvc resource testcase for Puppet Agent on Nexus
# devices.
...
# TestCase:
# ---------
# This is a AaaLoginCfgSvc resource test that tests for attribute default values
...
# Require UtilityLib.rb and AaaLoginCfgSvcLib.rb paths.
...
def create_aaalogincfgsvc_defaults(tests, id, title, string=false)
...
test_name "TestCase :: #{testheader}" do
  ...
  %w(default console).each do |title|
    ...
    tests[id][:desc] =
      '1.2 Check cisco_aaalogincfgsvc resource present on agent'
    test_resource(tests, id)

    tests[id][:desc] =
      "1.3 Apply default manifest with 'default' as a symbol in attributes"
    create_aaalogincfgsvc_defaults(tests, id, title, false)
