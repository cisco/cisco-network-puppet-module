...
# TestCase Name:
# -------------
# SnmpComm-Provider-NonDefaults.rb
#
...
# TestCase:
# ---------
# This is a SNMPCOMMUNITY resource test that tests for nondefault values for
...
# Steps 2-4 deal with cisco_snmp_community_resource creation and its
# verification using Puppet Agent and the switch running-config.
# Steps 5-7 deal with cisco_snmp_community resource deletion and its
# verification using Puppet Agent and the switch running-config.
#
...
result = 'PASS'
testheader = 'SNMPCOMMUNITY Resource :: All Attributes NonDefaults'

# @test_name [TestCase] Executes nondefaults testcase for SNMPCOMM Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Check snmpcomm instance presence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout,
                                          [%r{snmp-server community test group network-operator},
                                           %r{snmp-server community test use-acl aclname}],
                                          false, self, logger)
    end
