...
# TestCase Name: 
# -------------
# ExtendedVlan-Provider-NonDefaults.rb
#
...
# TestCase:
# ---------
# This is a EXTVLAN resource test that tests for nondefault values for 
...
# Steps 2-4 deal with cisco_vlan resource creation and its
# verification using Puppet Agent and the switch running-config.
# Steps 5-7 deal with cisco_vlan resource deletion and its
# verification using Puppet Agent and the switch running-config.
#
...
result = 'PASS'
testheader = "EXTVLAN Resource :: All Attributes NonDefaults"

# @test_name [TestCase] Executes nondefaults testcase for EXTVLAN Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step "TestStep :: Get resource nondefaults manifest from master" do 
    # Expected exit_code is 0 since this is a bash shell cmd.
    on(master, VlanLib.create_extvlan_manifest_nondefaults())
  ...
  step "TestStep :: Check vlan instance presence on agent" do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, [/vlan 2400/,
        /name DESCR-VLAN2400/],
        false, self, logger)
    end
