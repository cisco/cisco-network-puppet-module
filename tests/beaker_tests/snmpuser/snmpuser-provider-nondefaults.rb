...
# TestCase Name: 
# --------------
# SnmpUser-Provider-NonDefaults.rb
#
...
# TestCase:
# ---------
# This is a SNMPUSER resource test that tests for nondefault values for 
...
result = 'PASS'
testheader = "SNMPUSER Resource :: All Attributes Nondefaults"

# @test_name [TestCase] Executes nondefaults testcase for SNMPUSER Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step "TestStep :: Check snmpuser instance presence on agent" do
    # Expected exit_code is 0 since this is a vegas shell cmd.
    # Flag is set to false to check for presence of RegExp pattern in stdout.
    cmd_str = UtilityLib.get_vshell_cmd("show running-config snmp")
    on(agent, cmd_str) do
    ...
    logger.info("Check snmpuser instance presence on agent :: #{result}")
  end
