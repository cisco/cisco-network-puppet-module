...
# TestCase:
# ---------
# This is a OSPF resource test that tests for 'ensure' attribute with
...
result = 'PASS'
testheader = 'OSPF Resource :: Ensurability'

# @test_name [TestCase] Executes ensurability testcase for OSPF Resource.
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Check cisco_ospf resource presence on agent' do
    ...
    logger.info("Check cisco_ospf resource presence on agent :: #{result}")
  end

  # @step [Step] Checks ospf instance on agent using switch show cli cmds.
  step 'TestStep :: Check ospf instance presence on agent' do
    # Expected exit_code is 0 since this is a vegas shell cmd.
    # Flag is set to false to check for presence of RegExp pattern in stdout.
    cmd_str = UtilityLib.get_vshell_cmd('show running-config section ospf')
    on(agent, cmd_str) do
    ...
    logger.info("Check ospf instance presence on agent :: #{result}")
  end
