...
# TestCase Name:
# -------------
# DomainName-Provider-Defaults.rb
#
# TestCase Prerequisites:
# -----------------------
# This is a domain_name resource testcase for Puppet Agent on Nexus devices.
...
# TestCase:
# ---------
# This is a domain_name resource test that tests for default value for
# 'ensure' attribute of a domain_name resource.
#
...
# with the domain_name resource and its verification using Puppet Agent and the
...
# Require UtilityLib.rb and DomainNameLib.rb paths.
require File.expand_path('../../lib/utilitylib.rb', __FILE__)
require File.expand_path('../domain_namelib.rb', __FILE__)
...
test_name "TestCase :: #{testheader}" do
  ...
  # @step [Step] Checks domain_name resource on agent using resource cmd.
  ...
  # @step [Step] Checks domain_name instance on agent using switch show cli
  # cmds.
  ...
  # @step [Step] Requests manifest from the master server to the agent.
  step 'TestStep :: Get resource unknown manifest from master' do
    # Expected exit_code is 0 since this is a bash shell cmd.
    on(master, DomainNameLib.create_domain_name_manifest_negative)

    # Expected exit_code is 1 since this is a puppet agent cmd will error.
    cmd_str = UtilityLib.get_namespace_cmd(agent, UtilityLib::PUPPET_BINPATH +
                                           'agent -t', options)
    on(agent, cmd_str, acceptable_exit_codes: [1])

    logger.info("Get resource unknown manifest from master :: #{result}")
  end
  ...
  # @step [Step] Requests manifest from the master server to the agent.
  step 'TestStep :: Get invalid domain_name name manifest from master' do
    # Expected exit_code is 0 since this is a bash shell cmd.
    on(master, DomainNameLib.create_domain_name_manifest_name_invalid)
    # Expected exit_code is 6 since this is a puppet agent cmd will have a
    # change and also error.
    cmd_str = UtilityLib.get_namespace_cmd(agent, UtilityLib::PUPPET_BINPATH +
                                           'agent -t', options)
  ...
  step 'TestStep :: Check domain_name resource absence on agent' do
    ...
    # Flag is set to true to check for absence of RegExp pattern in stdout.
    cmd_str = UtilityLib.get_namespace_cmd(agent, UtilityLib::PUPPET_BINPATH +
      'resource domain_name a.b.c', options)
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, { 'ensure' => 'present' },
                                          true, self, logger)
    end
  ...
  step 'TestStep :: Check domain_name instance absence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, [/domain-name a\.b\.c/],
                                          true, self, logger)
    end
