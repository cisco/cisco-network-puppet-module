...
# -----------------------
# This is a Puppet bridge_domain_vni resource testcase for Puppet Agent on
...
def puppet_resource_cmd
  cmd = PUPPET_BINPATH + 'resource cisco_bridge_domain_vni'
...
def build_manifest_bridge_domain(tests, id)
  ...
  node 'default' {
    cisco_bridge_domain_vni { '#{tests[id][:title_pattern]}':
      #{state}
      #{tests[id][:manifest_props]}
    }
  }
...
test_name "TestCase :: #{testheader}" do
  ...
  test_harness_bridge_domain(tests, 'non_default_properties_change_member_vni')

  id = 'non_default_properties_change_member_vni'
  tests[id][:desc] = '2.2 Cleanup Bridge Domain'
  tests[id][:ensure] = :absent
  test_harness_bridge_domain(tests, id)

  logger.info("\n#{'-' * 60}\nSection 3. Non Default Property Testing")
  test_harness_bridge_domain(tests, 'non_default_properties_random_member_vni')

  id = 'non_default_properties_random_member_vni'
  tests[id][:desc] = '3.2 Cleanup Bridge Domain'
  tests[id][:ensure] = :absent
  test_harness_bridge_domain(tests, id)
end
