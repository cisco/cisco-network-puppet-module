...
# TestCase Name:
# -------------
# TacacsServerGroup-Provider-Defaults.rb
#
# TestCase Prerequisites:
# -----------------------
# This is a tacacs_server_group resource testcase for Puppet Agent on Nexus devices.
...
# TestCase:
# ---------
# This is a tacacs_server_group resource test that tests for default value for
# 'ensure' attribute of a tacacs_server_group resource.
#
...
require File.expand_path('../tacacs_server_grouplib.rb', __FILE__)
...
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Setup switch for provider' do
    ...
    cmd_str = UtilityLib.get_vshell_cmd('conf t ; tacacs-server host 2.2.2.2')
    on(agent, cmd_str)
    cmd_str = UtilityLib.get_vshell_cmd('conf t ; tacacs-server host 3.3.3.3')
    on(agent, cmd_str)
    cmd_str = UtilityLib.get_vshell_cmd('conf t ; tacacs-server host 4.4.4.4')
    on(agent, cmd_str)
  ...
  # @step [Step] Checks tacacs_server_group resource on agent using resource cmd.
  step 'TestStep :: Check tacacs_server_group resource presence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, { 'ensure' => 'present' },
                                          false, self, logger)
      UtilityLib.search_pattern_in_output(stdout, { 'servers' => '\[\'2.2.2.2\', \'3.3.3.3\'\]' },
                                          false, self, logger)
    end
  ...
  step 'TestStep :: Check tacacs_server_group resource presence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, { 'ensure' => 'present' },
                                          false, self, logger)
      UtilityLib.search_pattern_in_output(stdout, { 'servers' => '\[\'2.2.2.2\', \'4.4.4.4\'\]' },
                                          false, self, logger)
    end
  ...
  step 'TestStep :: Check tacacs_server_group resource presence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout, { 'ensure' => 'present' },
                                          false, self, logger)
      UtilityLib.search_pattern_in_output(stdout, { 'servers' => '\[\'unset\'\]' },
                                          false, self, logger)
    end
