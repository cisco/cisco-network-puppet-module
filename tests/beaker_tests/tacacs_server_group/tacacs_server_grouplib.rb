...
###############################################################################
# Tacacs Server Group Utility Library:
# ---------------------
# tacacs_server_grouplib.rb
#
# This is the utility library for the Tacacs Server Group provider Beaker test cases
# that contains the common methods used across the Tacacs Server Group testsuite's
...
#
# Every Beaker Tacacs Server Group test case that runs an instance of Beaker::TestCase
...
# A library to assist testing tacacs_server_group resource
module TacacsServerGroupLib
  # Group of Constants used in negative tests for tacacs_server_group provider.
...
EOF"
  ...
  # @param none [None] No input parameters exist.
  # @result none [None] Returns no object.
  def self.create_tacacs_server_group_manifest_present
    manifest_str = "cat <<EOF >#{PUPPETMASTER_MANIFESTPATH}
node default {
...
EOF"
  ...
  # @param none [None] No input parameters exist.
  # @result none [None] Returns no object.
  def self.create_tacacs_server_group_manifest_present_change
    manifest_str = "cat <<EOF >#{PUPPETMASTER_MANIFESTPATH}
node default {
  tacacs_server_group { 'red':
    ensure    => 'present',
...
EOF"
  ...
  # @param none [None] No input parameters exist.
  # @result none [None] Returns no object.
  def self.create_tacacs_server_group_manifest_present_servers_absent
    manifest_str = "cat <<EOF >#{PUPPETMASTER_MANIFESTPATH}
node default {
  tacacs_server_group { 'red':
    ensure    => 'present',
...
EOF"
  ...
  # @param none [None] No input parameters exist.
  # @result none [None] Returns no object.
  def self.create_tacacs_server_group_manifest_absent
    manifest_str = "cat <<EOF >#{PUPPETMASTER_MANIFESTPATH}
node default {
    tacacs_server_group {'red':
      ensure => absent,
