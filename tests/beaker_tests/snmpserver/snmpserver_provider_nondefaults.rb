...
# TestCase Name:
# -------------
# SnmpServer-Provider-NonDefaults.rb
#
...
# TestCase:
# ---------
# This is a SNMPSERVER resource test that tests for nondefault values for
...
# Steps 2-4 deal with cisco_snmp_server_resource change and its
# verification using Puppet Agent and the switch running-config.
# Steps 5-7 deal with cisco_snmp_server_resource defaults and its
# verification using Puppet Agent and the switch running-config.
#
...
result = 'PASS'
testheader = 'SNMPSERVER Resource :: All Attributes NonDefaults'
...
test_name "TestCase :: #{testheader}" do
  ...
  step 'TestStep :: Check snmpserver instance presence on agent' do
    ...
    on(agent, cmd_str) do
      UtilityLib.search_pattern_in_output(stdout,
                                          [%r{snmp-server aaa-user cache-timeout 1000},
                                           %r{snmp-server packetsize 2500},
                                           %r{snmp-server contact user1},
                                           %r{snmp-server location rtp}],
                                          false, self, logger)
    end
